...
* **Answer:**
  ...
  We can see that the Chinstrap species doesn't follow the same linear pattern as the other two species, it is above them, meaning that species has a different relationship between its flipper and bill lengths. If species had no effect on the relationship then they would all follow the same pattern. We can also tell that the sizes of the different species are noticeably different when viewing the samples distributions and their relative location in the scatter plot.
...
* **(e)**
    ...
    ```{r}
      ...
      T2 <- 100 * t(sample_mean_vector - mu) %*% solve(sample_cov_matrix) %*% (sample_mean_vector - mu)
...
* **(f)**
    ...
    ```{r}
      ...
      for (b in 1:B) {
        ...
        T2[b] <- 100 * t(sample_mean_vector - mu) %*% solve(sample_cov_matrix) %*% (sample_mean_vector - mu)
      }
